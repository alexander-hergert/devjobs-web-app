...
const Jobs = () => {
  ...
  return (
    <>
      <section className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-3 md:gap-x-[10px]
       xl:gap-x-[25px] gap-y-[60px] md:w-[690px] xl:w-[1100px] md:mt-[30px] xl:mt-[66px]">
        {jobs?.map((job) => (
          <Job key={job.job_id} job={job} />
        ))}
      </section>
      <button className="btn m-auto border-none my-12 block max-md:w-[141px] md:w-[121px] capitalize text-white bg-[#5762e0] hover:bg-info">
        Load More
      </button>
    </>
  );
